...
pub fn status() -> Result<()> {
    ...
    if !has_cloudflare_session(&credentials) {
        ...
        println!("API:    {api_base_url}");
        println!(
            "Note:   Stored credentials are from a removed auth flow. Run `ai-stats auth login` again."
        );
        return Ok(());
    }
...
pub fn get_or_refresh_token() -> Result<Option<String>> {
    ...
    let refresh_token = credentials
        ...
        .context("Cloudflare refresh token missing; run `ai-stats auth login`")?;
    ...
    let response = match response {
        ...
        Err(ureq::Error::Status(code, response)) => {
            ...
            if code == 400 || code == 401 {
                ...
                bail!("Cloudflare device session expired. Please run 'ai-stats auth login' again.");
            }
...
fn ensure_cloudflare_session(path: &Path, credentials: &AuthCredentials) -> Result<()> {
    ...
    let _ = std::fs::remove_file(path);
    bail!("Stored credentials use a removed auth flow. Please run `ai-stats auth login` again.")
}
