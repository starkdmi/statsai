...
impl Server {
    ...
    where
        A: ToSocketAddrs,
    {
        Self::http_with_limits(addr, crate::http::limits::HttpLimits::default())
    }

    /// Builds a server with explicit limits. Production callers use [`Server::http`].
    pub fn http_with_limits<A>(
        addr: A,
        limits: crate::http::limits::HttpLimits,
    ) -> Result<Server, Box<dyn Error + Send + Sync + 'static>>
    where
        A: ToSocketAddrs,
    {
        Server::new_with_limits(
            ServerConfig {
    ...
    pub fn new(config: ServerConfig) -> Result<Server, Box<dyn Error + Send + Sync + 'static>> {
        Self::new_with_limits(config, crate::http::limits::HttpLimits::default())
    }
    ...
    pub fn new_with_limits(
        ...
        Self::from_listener_with_limits(listener, config.ssl, limits)
    }
    ...
    pub fn from_listener<L: Into<Listener>>(
        ...
        Self::from_listener_with_limits(
            listener,
            ssl_config,
            crate::http::limits::HttpLimits::default(),
        )
    }
    ...
    pub fn from_listener_with_limits<L: Into<Listener>>(
        ...
        type SslContext = ();
        #[cfg(any(feature = "ssl-openssl", feature = "ssl-rustls"))]
        type SslContext = crate::http::tiny_http::ssl::SslContextImpl;
        ...
        let messages = MessagesQueue::with_capacity(limits.max_queued_requests);
        let active_connections = Arc::new(AtomicUsize::new(0));

        let inside_close_trigger = close_trigger.clone();
        let inside_messages = messages.clone();
        let inside_active = Arc::clone(&active_connections);
        thread::spawn(move || {
            ...
            while !inside_close_trigger.load(Relaxed) {
                let accepted = match server.accept() {
                    Ok((sock, _)) => sock,
                    Err(error) => {
                ...
                if !admit_connection(&inside_active, limits.max_active_connections) {
                    accepted.shutdown(Shutdown::Both).ok();
                    continue;
                }
                let guard = ActiveConnection {
                    count: Arc::clone(&inside_active),
                };
                let timeout_socket = match accepted.try_clone() {
                    Ok(socket) => socket,
                    Err(error) => {
                        drop(guard);
                        log::error!("Error cloning accepted client: {}", error);
                        inside_messages.push(error.into());
                        break;
                    }
                };
                let (read_closable, write_closable) = util::RefinedTcpStream::new(accepted);
                let client =
                    ClientConnection::new(write_closable, read_closable, timeout_socket, limits);
                ...
                tasks_pool.spawn(Box::new(move || {
                    ...
                    // One outstanding request per connection. The next read waits
                    // until this request is answered or dropped.
                    let (sender, receiver) = mpsc::channel();
                    for request in client {
                        if !messages.try_push(request.with_notify_sender(sender.clone()).into()) {
                            break;
                        }
...
fn admit_connection(active: &AtomicUsize, limit: usize) -> bool {
    loop {
        let current = active.load(Acquire);
        if current >= limit {
            return false;
        }
        if active
