    ...
    T: Send,
{
    ...
    /// Pushes an element when the queue is below its limit.
    ///
    /// The length check and the push share one lock, so concurrent producers
    /// cannot exceed the cap.
    pub fn try_push(&self, value: T) -> bool {
        let mut queue = self.queue.lock().unwrap();
        let queued = queue
            .iter()
            .filter(|item| matches!(item, Control::Elem(_)))
            .count();
